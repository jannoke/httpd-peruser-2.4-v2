...
%global mpm event
%else
%global mpm prefork peruser
%endif

Summary:              Apache HTTP Server
Name:                 httpd
Version:              2.4.62
Release:              1%{?dist}.3.peruser.040.v16.taavi
...
#DataCode Peruser (httpd 2.4 compatible, SSL parts disabled)
...
%changelog
* Fri Mar 07 2025 Janno Sannik <janno@kood.ee> - 2.4.62-1.3.peruser.040.v16
- Fix peruser patch for httpd 2.4 API compatibility:
  - Update ap_mpm_run() to use ap_hook_mpm() mechanism
  - Update ap_mpm_query() to use ap_hook_mpm_query() mechanism
  - Add peruser_get_name() and ap_hook_mpm_get_name() registration
  - Replace unixd_killpg() with ap_unixd_killpg()
  - Replace unixd_set_proc_mutex_perms() with ap_unixd_set_proc_mutex_perms()
  - Replace unixd_setup_child() with ap_unixd_setup_child()
  - Replace unixd_config with ap_unixd_config
  - Fix accept mutex creation to use ap_proc_mutex_create with AP_ACCEPT_MUTEX_TYPE
  - Add ap_mutex_register() call in pre_config
  - Remove ap_graceful_stop_signalled() (removed in httpd 2.4)
  - Remove ap_wait_or_timeout() local implementation (provided by httpd core)
  - Remove CORE_PRIVATE define (removed in httpd 2.4)
  - Remove UNIX_DAEMON_COMMANDS (moved to mod_unixd in httpd 2.4)
  - Fix ap_reclaim_child_processes() signature (new callback parameter)
  - Disable SSL routing (ssl API changed too much, disabled for now)
  - Add util_mutex.h and mod_unixd.h includes

* Wed Feb 19 2025 Janno Sannik <janno@kood.ee> - 2.4.62-1.2.peruser.040.v16
- Initial non-working peruser patch added.
